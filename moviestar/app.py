...
client = MongoClient('mongodb://admin:admin@43.200.7.212', 22)
db = client.dbjungle
...
# API #5: 휴지통에 버려진 영화 목록을 반환합니다.
@app.route('/api/list/trash', methods=['GET'])
def show_movies_trashed():
    # client 에서 요청한 정렬 방식이 있는지를 확인합니다. 없다면 기본으로 좋아요 순으로 정렬합니다.
    sortMode = request.args.get('sortMode', 'likes')

    # 1. db에서 trashed 가 False인 movies 목록을 검색합니다. 주어진 정렬 방식으로 정렬합니다.
    # 참고) find({},{}), sort()를 활용하면 됨.
    #      개봉일 순서 정렬처럼 여러 기준으로 순서대로 정렬해야되는 경우 sort([('A', 1), ('B', 1)]) 처럼 줄 45343수 있음.
    #    TODO: 다음 코드에서 likes로 정렬이 정상동작하도록 직접 수정해보세요!!!
    if sortMode == 'likes':
        # movies = list(db.movies.find({'trashed': False}, {}))
...
# API #6: 영화를 휴지통에서 복구합니다.
@app.route('/api/restore', methods=['POST'])
def restore_movie():
    movie_id = request.json['movie_id']
    result = db.movies.update_one({'_id':ObjectId(movie_id)},{'$set':{'trashed':False}})
    if result.modified_count == 1:
        return jsonify({'result': 'success'})
    else:
        return jsonify({'result': 'failure'})
